...
> 目標：給空一個偏向「深度推理」的模型檔位

- 建議主要模型：**OpenAI o3-mini 類推理模型**
  - 用途：複雜決策、系統設計、長文本分析
  - 風格：允許較長思考鏈，但輸出整理要乾淨
- 建議備用模型：**OpenAI gpt-4.1-mini 類通用模型**
  - 用途：問題不那麼複雜、想省額度或走快一點的時候

在 OpenClaw gateway 裡，建議：
- 給空綁一個 profile，例如 `agent: 空` → 預設模型指向 o3-mini（實際 model id 依你的 gateway 設定）
